...
These methods have been developed in response to trying to implement the popReconstruct model at scale with varying quality of input data but have not yet been published.
...
Level 3 of popReconstruct can be rewritten with the linear splines included. This isn't clear with the notation used but each of the year specific and age specific knots can be different for each component (srb, asfr, etc.).
...
g_{l,a,t} &= g^*_{l,a,t} + \mathbf{g}^f_a \delta^g_{k_a,k_t} {\mathbf{B}^{g}_t}^\intercal \end{aligned}$$
...
* reduce computation time since there are less parameters to be estimated.
